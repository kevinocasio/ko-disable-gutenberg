<?php
/**
 * Plugin Name: KO Disable Gutenberg
 * Plugin URI:  https://kevinocasio.com/tools/ko-disable-gutenberg/
 ...
 * Text Domain: ko-disable-gutenberg
 */
...
define('KO_DG_VERSION', '1.0.0');
define('KO_DG_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('KO_DG_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ko_dg_brand_url($path = '/tools/') {
 ...
 */
function ko_dg_activate()
{
	if (get_option('ko_dg_enabled') === false) {
		update_option('ko_dg_enabled', '1');
	}
}
register_activation_hook(__FILE__, 'ko_dg_activate');

/**
 * 2. Core Logic: Disable Block Editor & Restore Classic Editor
 */
function ko_dg_is_enabled()
{
	return (int) get_option('ko_dg_enabled', '1') === 1;
}

// Disable Gutenberg for posts
add_filter('use_block_editor_for_post', function ($use_block_editor) {
	return ko_dg_is_enabled() ? false : $use_block_editor;
}, 10);
...
add_action('admin_menu', 'ko_dg_admin_menu');

/**
 * 4. AJAX Handler for Instant Dashboard Toggle
 */
function ko_dg_ajax_toggle_status()
{
	check_ajax_referer('ko_dg_toggle_nonce', 'nonce');
	...
	update_option('ko_dg_enabled', $enabled);
...
add_action('wp_ajax_ko_dg_toggle_status', 'ko_dg_ajax_toggle_status');

/**
 * 5. Action Link on Plugins Screen
 */
function ko_dg_settings_action_link($links)
{
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ko_dg_settings_action_link');

/**
 * 6. Conditional Admin Asset Enqueuing (Strict Anti-Bloat)
 */
function ko_dg_enqueue_admin_assets($hook)
{
	if ('toplevel_page_ko-plugins-main' !== $hook) {
		return;
	}
	$ver = file_exists(KO_DG_PLUGIN_DIR . 'assets/admin.css') ? filemtime(KO_DG_PLUGIN_DIR . 'assets/admin.css') : KO_DG_VERSION;
	wp_enqueue_style('ko-dg-admin-css', plugins_url('assets/admin.css', __FILE__), array(), $ver);
}
add_action('admin_enqueue_scripts', 'ko_dg_enqueue_admin_assets');
...
if (!function_exists('ko_plugins_suite_dashboard_html')) {
	function ko_plugins_suite_dashboard_html()
	{
		...
		$hub_url = ko_dg_brand_url('/tools/');
		$author_url = ko_dg_brand_url('/');
		...
		<div class="wrap ko-dash-wrap">
			...
			<div class="ko-dash-grid">
				<?php foreach ($suite_plugins as $plugin):
					...
					$plugin_url = ko_dg_brand_url('/tools/' . $plugin['slug'] . '/');
					?>
